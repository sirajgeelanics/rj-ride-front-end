...
export const Sidebar: React.FC<SidebarProps> = ({ open = false, onClose }) => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed inset-0 bg-[#072D62]/40 backdrop-blur-sm z-40 lg:hidden transition-opacity duration-300 ${
      ...
      Below `lg` it's an off-canvas slide-in panel; at `lg`+ it's the original static rail. */}
      <aside
        className={`w-60 bg-ops-card border-r border-border h-screen flex flex-col fixed inset-y-0 left-0 z-50 transition-transform duration-300 lg:static lg:translate-x-0 lg:z-auto ${
